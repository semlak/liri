...
module.exports = class MySpot {
	...
	prepareLiriPrint(songData) {
		...
		return (this.liriPrint.go(songData, prettyKeyNames));
		let output = keysToPrint.map(key =>prettyKeyNames[key] + ": " + songData[key]).join("\n");
		return (output);
	}

	spotifyThisSong(args, loggerCallback) {
		...
		this.client.search({type: 'track', query: trackQuery})
			.then(function(response) {
				...
				data.forEach(function(item, i) {
					...
					loggerCallback((this.prepareLiriPrint(songData)));
